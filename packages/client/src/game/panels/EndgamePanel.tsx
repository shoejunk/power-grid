...
export function EndgamePanel(): JSX.Element {
  ...
  return (
    ...
    >
      ...
      {standings.length === 0 ? (
        <p className="pg-caption">The evaluation has not been recorded yet.</p>
      ) : (
        <div className="pg-gfinal" role="list">
          {standings.map((row, index) => {
            ...
            return (
              ...
              >
                <span className="pg-gfinal__rank pg-numeral">{index + 1}</span>
                ...
                <span className="pg-gfinal__calc">
                  <span className="pg-gfinal__big pg-numeral">{row.citiesSupplied}</span>
      ...
      <div className="pg-gsummary">
        <Stat label="Threshold" value={<span className="pg-numeral">{threshold}</span>} />
        <Stat label="Rounds played" value={<span className="pg-numeral">{state.round}</span>} />
        <Stat label="Final Step" value={<span className="pg-numeral">{state.step}</span>} />
      </div>
    </PhaseShell>
  );
}
