  ...
  return (
    ...
    >
      <div className="pg-gsummary">
        <Stat label="Network" value={<span className="tt-numeral">{network}</span>} />
        <Stat label="Generation" value={<span className="tt-numeral">{capacity}</span>} />
        <Stat label="Payout" value={<Money value={payout} tone="gain" />} />
      </div>

      <span className="tt-overline pg-gsectionlabel">Which plants run</span>
            ...
            >
              <span className="pg-gcombo__plants">
                {o.plantIds.length === 0 ? (
                  ...
                  o.plantIds.map((id) => (
                    <span key={id} className="pg-gcombo__chip tt-numeral">
                      {id}
                    </span>
                  ))
                )}
              ...
              <span className="pg-gcombo__cap">
                <b className="tt-numeral">{o.capacity}</b> generated
              </span>
      ...
      {option && option.plantIds.length > 0 ? (
        <div className="pg-gburn">
          <span className="tt-overline">Fuel burnt</span>
          <span className="pg-gburn__tokens">
            {burnTotal === 0 ? (
              <span className="tt-caption">Nothing — ecological plants burn no fuel (§1).</span>
            ) : (
      ...
      <div className="pg-gsupply">
        <span className="tt-overline">Cities to supply</span>
        <NumberStepper
...
function PaymentTable({ highlight }: { highlight: number | null }): JSX.Element {
  return (
    <div className="pg-gpay">
      <span className="tt-overline pg-gsectionlabel">Payment summary (§9.1)</span>
