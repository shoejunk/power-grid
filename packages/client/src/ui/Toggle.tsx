...
export function Toggle({
  ...
  return (
    <div
      className={['pg-toggle', className ?? ''].filter(Boolean).join(' ')}
    ...
    >
      <button
        ...
        disabled={disabled}
        className="pg-toggle__switch"
        onClick={() => onChange(!checked)}
      >
        <span className="pg-toggle__track" aria-hidden="true">
          <motion.span
            className="pg-toggle__knob"
      ...
      <div className="pg-toggle__text">
        ...
        {description !== undefined ? (
          <span className="pg-toggle__desc" id={descId}>
            {description}
          </span>
        ) : null}
      </div>
