...
export function LoadingSpinner({
  ...
  if (!label) {
    return (
      <>
        {ring}
        <span className="pg-sr-only" role="status">
          Loading
        </span>
      </>
    );
  ...
  return (
    <div
      className={['pg-loading', className ?? ''].filter(Boolean).join(' ')}
      style={style}
      role="status"
      aria-live="polite"
    >
      {ring}
      <span className="pg-loading__label">{label}</span>
    </div>
  );
}
