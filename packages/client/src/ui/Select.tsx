...
export function Select<T extends string>({
  ...
  return (
    <div className={['pg-field', className ?? ''].filter(Boolean).join(' ')}>
      {label !== undefined ? (
        <span className="pg-field__label" id={`${id}-label`}>
          {label}
        </span>
      ) : null}

      <div className="pg-select" ref={rootRef}>
        <button
          ref={triggerRef}
          type="button"
                  ...
                  >
                    <span>
                      ...
                      {option.detail !== undefined ? (
                        <span className="pg-panel__subtitle">{option.detail}</span>
                      ) : null}
                    </span>
