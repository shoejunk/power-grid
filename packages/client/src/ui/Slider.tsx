...
export function Slider({
  ...
  return (
    <div className={['pg-slider', className ?? ''].filter(Boolean).join(' ')}>
      <div className="pg-slider__head">
        <label className="pg-field__label" htmlFor={id}>
          {label}
        </label>
        <span className="pg-slider__value">
          {format ? format(value) : value}
          {unit !== undefined ? <span className="pg-money__unit"> {unit}</span> : null}
        </span>
      </div>

      <div className="pg-slider__track">
        <motion.div
          className="pg-slider__fill"
          animate={{ width: `${pct}%` }}
          transition={springSnappy}
        />
        <motion.div
          className="pg-slider__thumb"
          animate={{ left: `${pct}%` }}
          transition={springSnappy}
        />
        <input
          id={id}
          className="pg-slider__input"
          type="range"
          min={min}
          max={max}
          step={step}
      ...
      {showTicks ? (
        <div className="pg-slider__ticks" aria-hidden="true">
          {ticks.map((t) => (
            <span key={t}>{t}</span>
          ))}
        </div>
