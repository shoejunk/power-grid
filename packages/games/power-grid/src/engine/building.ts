...
export function validateBuildCity(
  ...
  const player = getPlayer(state, playerId);
  if (player.phaseStatus !== 'acting') return fail('You have finished building and must choose production');
...
export function validateUndoLastBuild(state: GameState, playerId: PlayerId): ValidationResult {
  ...
  if (getPlayer(state, playerId).phaseStatus !== 'acting') {
    return fail('You have finished building and must choose production');
  }
