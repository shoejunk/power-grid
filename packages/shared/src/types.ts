...
export interface LobbyState {
  ...
  settings: GameSettings;
  players: LobbyPlayer[];
  /** True once the host has started and `GameState` exists. */
  started: boolean;
}
...
export type ClientMessage =
  ...
  | { t: 'createGame'; name: string; settings: Omit<GameSettings, 'seed'> & { seed?: string } }
...
export type ServerMessage =
  | { t: 'welcome'; sessionToken: string; playerId: PlayerId }
  | { t: 'lobby'; lobby: LobbyState }
  | { t: 'state'; state: GameState }
  | { t: 'patch'; ops: unknown[]; version: number }
